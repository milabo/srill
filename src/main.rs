...
async fn get_channel_lambda_pairs(args: &Args) -> anyhow::Result<(Vec<ChannelLambdaPair>, String, mode::Mode)> {
    let mut pairs = Vec::new();
    ...
    if let Some(config_path) = &args.config {
        ...
        if let Some(config_mode) = config.mode {
            mode = match config_mode.as_str() {
                "sqs" => mode::Mode::Sqs,
                _ => return Err(anyhow::anyhow!("Unsupported mode in config: {}", config_mode)),
            };
        }
